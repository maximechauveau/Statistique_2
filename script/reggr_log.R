...
### Conversion Winner en factor pour la regression logistique
...
### Regression logostique

reg <- glm(df_regr_log$Winner ~ df_regr_log$R_fighter, family = 'binomial')

summary(reg)

### Choix catégorie
freq(df_regr_log$weight_class)
...
summary(reg_Nb_match)

### Test

dftest <- df_regr_log[,c("Winner", "R_Nb_total_combat", "B_Nb_total_combat")]

dftest$ratio_nb_combat <- dftest$R_Nb_total_combat / dftest$B_Nb_total_combat

table(dftest$ratio_nb_combat)

dftest <- dftest %>% filter(ratio_nb_combat != Inf)


reg_Nb_match <- glm(Winner ~ ratio_nb_combat, family = 'binomial', data = dftest)
